  ...
  localized do
    scope :cmor_user_area_frontend_engine do
      resource :current_user, only: [:show, :edit, :update] do
        ...
        resource :two_factor_authentication_setup_service, only: [:new, :create] if Cmor::Core::Settings.get("cmor_user_area/tfa.enable")
      end
      resource :current_user, only: [:new, :create] if Cmor::Core::Settings.get("cmor_user_area/user.enable_registrations")
      resource :current_user, only: [:destroy] if Cmor::Core::Settings.get("cmor_user_area/user.allow_users_to_destroy_self")
      ...
      resource :user_two_factor_authentications, only: [:new, :create] if Cmor::Core::Settings.get("cmor_user_area/tfa.enable")
